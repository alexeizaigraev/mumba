...
def get_kabinet_pereezd_new_data():
...
data = get_kabinet_pereezd_new_data()

for u in data:
    koatu2 = mk_koatu2(u[3], u[-2], u[6])
    if not koatu2:
        p_magenta(f'no koatu2 {u[0]}')
    u = list(u)
    if u[2] == '':
        u[2] = 'Київська'
...
<HFILL>{now_date_kabinet()}</HFILL>
    ...
    ofname = KABINET_DIR + u[0] + '_pereezd_' + u[9] + '.xml'
    #say(ofname)
